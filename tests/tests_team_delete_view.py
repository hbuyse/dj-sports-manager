...
class TestTeamUpdateViewAsAnonymous(TestCase):
    ...
    def test_get_not_existing(self):
        """Tests."""
        r = self.client.get(reverse('dj-sports-manager:team-delete', kwargs={'slug': 'not-existing'}))

        self.assertEqual(r.status_code, 403)

    def test_get(self):
        """Tests."""
        r = self.client.get(reverse('dj-sports-manager:team-delete', kwargs={'slug': self.team.slug}))

        self.assertEqual(r.status_code, 403)

    def test_post(self):
        """Tests."""
        r = self.client.post(reverse('dj-sports-manager:team-delete', kwargs={'slug': self.team.slug}),
                             **self.team_info)
...
class TestTeamUpdateViewAsLogged(TestCase):
    ...
    def test_get_not_existing(self):
        ...
        r = self.client.get(reverse('dj-sports-manager:team-delete', kwargs={'slug': 'toto'}))
...
class TestTeamUpdateViewAsStaff(TestCase):
    ...
    def test_post(self):
        ...
        self.assertEqual(r.url, reverse('dj-sports-manager:teams-list'))
