...
class TestCategoryUpdateViewAsAnonymous(TestCase):
    ...
    def test_get_not_existing(self):
        """Tests."""
        r = self.client.get(reverse('dj-sports-manager:category-delete', kwargs={'slug': 'not-existing'}))

        self.assertEqual(r.status_code, 403)

    def test_get(self):
        """Tests."""
        r = self.client.get(reverse('dj-sports-manager:category-delete', kwargs={'slug': self.category.slug}))

        self.assertEqual(r.status_code, 403)

    def test_post(self):
        """Tests."""
        r = self.client.post(reverse('dj-sports-manager:category-delete', kwargs={'slug': self.category.slug}),
                             **self.category_info)
...
class TestCategoryUpdateViewAsLogged(TestCase):
    ...
    def test_get_not_existing(self):
        ...
        r = self.client.get(reverse('dj-sports-manager:category-delete', kwargs={'slug': 'toto'}))
...
class TestCategoryUpdateViewAsStaff(TestCase):
    ...
    def test_post(self):
        ...
        self.assertEqual(r.url, reverse('dj-sports-manager:categories-list'))
