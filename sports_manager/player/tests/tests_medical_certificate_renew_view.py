...
from sports_manager.player.forms import MedicalCertificateRenewForm
...
class TestMedicalCertificateUpdateViewAsLogged(TestCase):
    ...
    def test_get_right_account_player_existing_one_certificate_valid(self):
        ...
        print(r.context['object'])
        self.assertEqual(r.context, self.certif._object)
...
#                                                                                self.player.get('slug'), self.certif.pk), fetch_redirect_response=True)
#         self.certif.refresh_from_db()
