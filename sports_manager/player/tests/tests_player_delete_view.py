...
class TestPlayerDeleteViewAsAnonymous(TestCase):
    ...
    def test_get_player_other_owner_player_not_existing(self):
        ...
        We should get a 403 status code since the 'test_func' kicks in first.
        """
        r = self.client.get(reverse('sports-manager:player-delete', kwargs={'username': self.other_owner.get_username(), 'slug': self.other_player.slug + 'a'}))
        self.assertEqual(r.status_code, 403)
    ...
    def test_post_player_other_owner_player_not_existing(self):
        ...
        We should get a 403 status code since the 'test_func' kicks in first.
        """
        r = self.client.post(reverse('sports-manager:player-delete', kwargs={'username': self.other_owner.get_username(), 'slug': self.other_player.slug + 'a'}))
        self.assertEqual(r.status_code, 403)
...
class TestPlayerDeleteViewAsLogged(TestCase):
    ...
    def test_get_player_user_player_not_existing(self):
        ...
        self.assertTrue(self.client.login(username=self.user_info['username'], password=self.user_info['password']))
        r = self.client.get(reverse('sports-manager:player-delete', kwargs={'username': self.user.get_username(), 'slug': self.player.slug + 'a'}))
        self.assertEqual(r.status_code, 404)

    def test_get_player_user_owner_and_player_existing(self):
        """Get a 200 status code since the user access its own player."""
        self.assertTrue(self.client.login(username=self.user_info['username'], password=self.user_info['password']))
        r = self.client.get(reverse('sports-manager:player-delete', kwargs={'username': self.user.get_username(), 'slug': self.player.slug}))
    ...
    def test_post_player_user_player_not_existing(self):
        """Get a 404 status code since the user access its own pages but the 'player' does not exist."""
        self.assertTrue(self.client.login(username=self.user_info['username'], password=self.user_info['password']))
        r = self.client.post(reverse('sports-manager:player-delete', kwargs={'username': self.user.get_username(), 'slug': self.player.slug + 'a'}))
        self.assertEqual(r.status_code, 404)
